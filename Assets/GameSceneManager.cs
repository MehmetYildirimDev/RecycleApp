...
public class GameSceneManager : MonoBehaviour
{
    ...
    {
        if (isimDegisInput.text != null || isimDegisInput.text != "" || isimDegisInput.text != isimText.text)
        {
            //sadece bir yerin degerini degistirme
            reference.Child("OyunVerileri").Child(auth.CurrentUser.UserId).Child("KullaniciIsim").SetValueAsync(isimDegisInput.text);
        }
    }
        ...
        reference.Child("OyunVerileri").Child(auth.CurrentUser.UserId).GetValueAsync().ContinueWithOnMainThread(task => {
            ...
            else if (task.IsCompleted)
            {
                ...
                else
                {
                    ...
                    isimText.text = "Kullanýcý Adý:" + data.KullaniciIsim;
...
public class OyunData //tutmak istedigimiz degiskenler
{
